...
\Page{
	...
	\EndProof
	\Explain{
		This proof is bogus as the references integration, 
...
\Page{
	...
	\Theorem{JensensInequalityForConditionalExpectations}
	{
		...
		\NewLine \.
		\forall \aleph : \phi(f) \in L^1(X,\Sigma,\mu) \.
		\forall  g :  \CE(X,\Sigma,\pi,T,f) \. \NewLine \.
		\forall  h :  \CE(X,\Sigma,\pi,T,\phi(f)) \.  
		\phi(g) \le h \And \int \phi(g) \le \int \phi(f)
	}
	\ExplainFurther{ 1 Intuitively, conditional expectations are composed of small integrals,} 
	\Explain{ $\quad$ so ordinary  Jensens inequality can be applied to them}
	\Explain{ 2 This produces $\phi(g) \le h$ }	
	\Explain{ 3 Then use the equality of integrals to get $\int \phi(g) \le \int h  = \int \phi(f)$}
	\EndProof
	\\
	\Theorem{RoselliWillemInequalityForConditionalExpectations}
	{
		\NewLine ::		
		\forall (X,\Sigma,\mu)  \in \MEAS \.
		\forall \phi : \TYPE{ConvexFunction}(\Reals) \.
		...
		\forall \aleph : \phi(f) \in L^1(X,\Sigma,\mu) \.
		\forall  g :  \CE(X,\Sigma,\mu,T,f) \. \NewLine \.
		\forall  h :  \CE(X,\Sigma,\mu,T,\phi(f)) \.  
		\phi(g) \le h \And \int \phi(g) \le \int \phi(f)
	}
	\NoProof
	\\
	\Theorem{StrongProduct}
	{
		\forall (X,\Sigma,\pi) : \TYPE{Probability} \.
		\forall T \subset_\sigma X \.
		\forall f \in L_1(X,\Sigma,P) \.
		\forall h \in \BOR_{\pi|T}^*(X,T) \. \NewLine \.
		\forall g : \CE(X,\Sigma,\pi,T,f) \.
		\forall g' : \CE(X,\Sigma,\pi,T,|f|) \. \NewLine \.
		f h \in L_1(X,\Sigma,\pi) \iff 
		g' h \in L_1*(X,T,\pi|T) \And 
		\CE(X,\Sigma,\pi,T,fh,gh)
	}
	\NoProof
}
\newpage
\subsection{Structures and Transforamtions}
...
\Page{
	...
	\Explain{1 Let $\F$ be a disjoint family in $Y$ of $\nu$-nonzero sets}
	\Explain{2 Then $f^{-1}(\F)$ is a such cover for $X$ and $\mu$-nonzero sets}
	\Explain{3 This means that $f^{-1}(\F)$ is countable}
	\Explain{4 $f^{-1}$ is injective on $\F$}
	\Explain{4.1 Assume $A,B \in \F$ such that $A \neq B$ and $f^{-1}(A) = f^{-1}(B)$}
	\Explain{4.2 $\nu(A \cup B) = \nu(A) + \nu(B)$ as $AB = \emptyset$}
	\Explain{4.3 By measure preservation $\nu(A) + \nu(B) = \mu( f^{-1}(A \cap B) ) = 
		\mu(f^{-1}(A)) = \nu(A)$}
	\Exclaim{4.4 But $\nu(A) + \nu(B) > \nu(A)$}
	\Explain{4.4 So $\F$ must also be countable}
	\Explain{4.5 Thus, $(Y,T,\nu)$ is $\sigma$-finite}
	\EndProof
	\\
	\Theorem{AtomlessCodomain}
	{
		\NewLine ::	
		\forall (X,\Sigma,\mu),(Y,T,\nu) \in \MEAS \. 
		\forall f \in \MEAS^\#\Big((X,\Sigma,\mu),(Y,T,\nu)\Big) \. \NewLine \.
		\sFinite \And \Aless(Y,T,\nu) \.
		\Imply
		\Aless(X,\Sigma,\mu) 
	}
	\Explain{ 1 Assume $A \in \Sigma$ such that $\mu(A) > 0$}
	\Explain{ 2 Let $\F$ be a disjoint family in $Y$ of $\nu$-nonzero sets}
	\Explain{ 3 As $\nu$ is atomless it is possible to assume thet $\nu(F) < \mu(A)$ for 
		any $F \in \F$}
	\Explain{ 4Then $f^{-1}(\F)$ covers $X$}
	\Explain{ 5 So there must be $F \in \F$ such that $\mu(f^{-1}(F) \cap A) > 0$}
	\Explain{ 6 But  $\mu(f^{-1}(F) \cap A) \le \mu(f^{-1}(F)) = \nu(F) < \mu(A)$ }
	\Explain{ 7 Thus, $A$ is not an atom}
	\EndProof
	\\
	\Theorem{PurelyAtomicDomain}
	{
		\NewLine ::		
		\forall (X,\Sigma,\mu),(Y,T,\nu) \in \MEAS \. 
		\forall f \in \MEAS^\#\Big((X,\Sigma,\mu),(Y,T,\nu)\Big) \. \NewLine \.
	...
	\Explain{ 1 Assume $A \in T$ such that $\nu(A) > 0$}
	\Explain{ 2 As $\nu$ is semifinite there is a measurable 
		$B \subset A$ such that $0 < \nu(B) < \infty$ }
	\Explain{  3  Then $\mu(f^{-1}(B)) = \nu(B) > 0 $
	 	and as $\mu$ is purely atomic there is an atom 
	 	$E \subset f^{-1}(B)$ of $\mu$}
	 \Explain{ 4
	 	Define $\F = \{ F \in T :  F \subset B \And \mu(f^{-1}(A) \cap E) = 0 \}$}
	 \Explain{ 5
	 	$\F$ is closed under countable unions}
	 \Explain{ 6  So there is $G \in \F$ such that $\nu(F\setminus G) = 0$ for any $F \in \F$}
	 \Explain{ 7 Then $H =  B \setminus G$ is a an atom of $\nu$}
	 \Explain{ 7.1 $0 < \mu(E) = \mu(f^{-1}(H)) = \nu(H)$}
	 \Explain{ 7.2 On the other hand, assume there is $F \subset H$ such that $ 0 < \nu(F) < \nu(H)$ }
	 \Explain{ 7.3 But then $0 < \mu(E \cap f^{-1}(F)) < \mu(E)$, but this is imposible}
	 \EndProof
}\Page{
	...
	\Explain{ 3 If $A,B \in T$ are such that $f^{-1}(A) = f^{-1}(B)$ then $A \du B$ 
		has measure zero}
	\Explain{ 3.1 This is true as $f(X)$ is $\nu$-thick}
	\Explain{ 4 So  $\nu(A) = \nu(B)$}
	\Explain{ 5 So it must be possible to define $\mu(f^{-1}(A)) = \nu(A)$ }
	\Explain{ 6 Thne $\mu$ is a measure}
	\Explain{ 6.1 Obviously $\mu(\emptyset) = \nu(\emptyset) = 0$ } 
	\Explain{ 6.2 Assume $A$ is a disjoint sequence in $\Sigma$}
	\Explain{ 6.3 Select $B$ in a such way that $A = f^{-1}(A)$}	
	\Explain{ 6.4 Then $B$ are not necesserly disjoint}
	\Explain{ 6.5 But the sequence $C_n = B_n \setminus \bigcup^{n-1}_{i=1} B_i$}
	\Explain{ 6.6 And $A_n = f^{-1}(C_n)$}
	\Explain{ 6.7 So $\mu\left( \bigcup^\infty_{n=1} A_n  \right) = 
		\nu\left(\bigcup^\infty_{n=1} C_n \right) =  
		\sum^\infty_{n=1} \nu(C_n) = \sum^\infty_{n=1} \mu(A_n)$}
	\Explain{ 7 $f_* \mu = \nu$}
}
\newpage
\subsubsection{Sums}
\Page{
	...
	$\sum_{i\in I} f^+ \; d\mu_i(X)$ or $\sum_{i\in I} f^- \; d\mu_i(X)$
	 is finite}
	 \Explain{ 1.3 Generally $f^+ d \mu_i \le \sum_{i\in I} f^+ \; d\mu_i(X)$
	 	and $f^- d \mu_i \le \sum_{i\in I} f^- \; d\mu_i(X)$}
	 \Explain{ 1.4 So $f$ must be integrable with respect to individual measures $\mu_i$}
	 \Explain{ 2 $(\Leftarrow)$}
	 \Explain{ 2.1 Assume that $f \ge 0$}
	 \Explain{ 2.2 Approximate $f$ by simple functions 
	 	$\sigma_n(x) = \sum^{4^n}_{k=1} 2^{-n}[ f(x) \ge 2^{-1}k ] $}
	 \ExplainFurther{
			2.3 Then 
			$
				\int \sigma_n \; d \sum_{i \in I} \mu_i = 
				\sum^{4^n}_{k=1} \left(\sum_{i \in I} \mu_i\right)\{ x \in X : f(x) \ge 2^{-n}k  \}	=
				\sum_{i \in I} \sum^{4^n}_{k=1} \mu_i \{ x \in X : f(x) \ge 2^{-n}k  \}	 =$}
	\Explain{
			$	\quad = \sum_{i \in I} \int \sigma_n \;d \mu_i$}
	\Explain{ 2.4 So by taking supremas
		 $\int f \; d\sum_{i \in I} \mu_i = \sum_{i \in I} \int f \; d \mu_i$
	}
	\Explain{ 2.5 In the general case the assumptions provide the desired result}
	\EndProof
	\\	
	\Theorem{SumIntegral}
	{
		...
		\forall (Y,\Sigma,\mu) : I \to \CMS \.
		\forall \aleph : \forall i \in I \. Y_i = X \. \NewLine \. 
		\forall f  \in  \Integrable\left( X, \bigcap_{i \in I } \Sigma_i, \sum_{i\in I} \mu_i \right)
		\NewLine 
		\int f \; d\sum_{i \in I} \mu_i = \sum_{i \in I} \int f \; d \mu_i
	}
	\Explain{This follows from the previous deduction}
	\EndProof
	\\
	\Explain{
		This result can be seen as a special case of Fubbini's theorem}
}
\newpage
...
\Page{
	...
	\Explain{ $= \overline{\int} J(x)f(\phi(x)) \; d\mu(x)$}
	\Explain{ 2.1  Assume $g \ge f$}
	\Explain{ 2.2 Then $J \cdot \phi g \ge J \cdot \phi f$ as $J$ is non-negative}
	\Explain{ 2.3 So the inequality is justify}
	\Explain{ 3 The argument for lower integrals is dual}
	\EndProof
}\Page{
	\Theorem{RadonNikodymJacobianExistance}
	{
		\NewLine ::		
		\forall (X,\Sigma,\mu),(Y,T,\nu) : \sFinite \.
		\forall D \in \Null_\mu' \. \NewLine \. 
		\forall \phi : D \to Y \.
		\forall \aleph : \forall F \in T \. \phi^{-1}(F) \in \Sigma \.
		\forall \beth :  \forall F \in T \. \nu(T) > 0 \Imply \phi_* \mu(T) > 0 \. \NewLine \. 
		\exists \TYPE{RadonNikodumJacobian}\Big( (X,\Sigma,\mu), (Y,T,\nu), A, \phi \Big)
	}
	\Explain{1 The theorem holds in the case $D=X$ and $\nu$ is finite}
	\Explain{1.1 Define $\hat T = \phi^{-1} T$}
	\Explain{1.2 $\hat T$ is a $\sigma$-algebra}
	\Explain{1.2.1 $\emptyset = \phi^{-1}(\emptyset) \in \hat T$}
	\Explain{1.2.2 Assume $A \in \hat T$}
	\Explain{1.2.3 Then there is $C \in T$ such that $A = \phi^{-1}(C)$}
	\Explain{1.2.4 So $X\setminus A = \phi^{-1}(Y)\setminus\phi^{-1}(C) 
		= \phi^{-1}(T\setminus C) \in \hat T$}
	\Explain{1.2.5 Now consider a sequence $A : \Nat \to \hat T$}
	\Explain{1.2.6 Then again there is a sequence $C : \Nat \to T$ such that $A_n = \phi^{-1}(C_n)$}
	\Explain{1.2.7 $ \bigcap^\infty_{n=1} A_n = \bigcap^\infty_{n=1} \phi^{-1}(C_n) = 
		\phi^{-1}\left( \bigcap^\infty_{n=1} C_n \right) \in \hat T $}
	\Explain{1.3 Define $\hat \nu : \hat T \to \Reals_+$ by $\hat\nu(\phi^{-1}(A)) = \nu(A)$}
	\Explain{1.3.1 Assume $A,B \in T$ are such that $\phi^{-1}(A) = \phi^{-1}(B)$}
	\Explain{1.3.2 Obviously $\phi^{-1}(A \du B) = \emptyset$}
	\Explain{1.3.3 So $\mu(\phi^{-1}(A \du B) ) = 0$ and $\nu(A \du B) = 0$ by initial hypothesis}
	\Explain{1.3.4 This $\nu(A) = \nu(B)$ and $\hat \nu$ is well-defined}
	\Explain{1.4 $\hat \nu$ is a finite measure}
	\Explain{1.4.1  $\hat \nu(\emptyset) = \nu(\emptyset)$}
	\Explain{1.4.2 Consider a disjoint sequence $A : \Nat \hat T$}
	\Explain{1.4.3 Thene there is a sequence $C' : \Nat \to T$ such that $A_n = \phi^{-1}(C'_n)$}
	\Explain{1.4.4 The sequence $C'$ may not be disjoint, 
		so define $C_n = C'_n \setminus^{n-1}_{k=1} C'_k $}
	\Explain{1.4.5 Then the sequence $C_n$ is disjoint and still has property 
		 $A_n = \phi^{-1}(C_n)$}
	\Explain{1.4.6 So 
		$
			\hat \nu\left( \bigcap^\infty_{n=1} A_n \right) = 
			\nu\left( \bigcap^\infty_{n=1} C_n \right) =
			\sum^\infty_{n=1} \nu(C_n)  = 
			\sum^\infty_{n=1} \hat \nu(A_n)
		$
	}
	\Explain{1.5 Note, that $\hat \nu(A) > 0$ imply that $\mu(A) > 0$}
	\Explain{1.6 By principle of exhaustion theres is a function $f : X \to \Reals_{++}$such that 
		 $\int f \; d\mu < \infty$ }
	\Explain{1.7 Define $\hat \mu : \hat T \to \Reals_+$ by $\hat \mu(E) = \int_E f \; d \mu$  }
	\Explain{1.8 $\hat \mu$ is a finite measure}
	\Explain{1.9 $\hat \mu(A)$ implies that $\mu(A) = 0$ as $f > 0$}
	\Explain{1.10 So, by Radon-Nikodym theorem there is a density $g$ such that 
		$\hat \nu = g \;d \hat \mu$}
	\Explain{1.11  Thus,   	$\hat \nu = fg \; d\mu$
	}
	\Explain{1.12 Moreover, 
		$\nu(F) = \hat \nu(\phi^{-1}(F)) = \int f(x)g(x) \delta_x(\phi^{-1}(F)) \; d\mu$
		for any $F \in T$ 
	}
	\Explain{1.13 So write $J = fg$ and use it as Radon-Nocodym Jacobian}
	\Explain{2 In case $D \neq X$ we may use extension $\hat \phi$ of $\phi$
		which is constant on $X \setminus D$}
}\Page{ 
	\Explain{
		3 In case $\nu$ is $\sigma$-finite construct a $\nu$-finite partition $\F$ 
		  and construct Jacobian separately for each $F \in \F$}
	\EndProof
	\\
	\DeclareType{InverseJacobian}
	{
		\prod  (X,\Sigma,\mu),(Y,T,\nu)  \in \MEAS\.
		\prod A \in \Null'_\mu  \.
		\prod B \in \Null'_\nu \.
	...
	\DefineType{I}{InverseJacobian}
	{\forall E \in T \. \int_E I \; d\nu =  \phi_*\mu(E)}
	\\	
	\Theorem{InverseChangeOfVariable}
	{
		\NewLine :: 		
		\forall (X,\Sigma,\mu),(Y,T,\nu) \in \MEAS \.
		\forall A \in \Null'_\mu \.
		\forall B \in \Null'_\nu
		\forall \phi : A \to Y \.
		\forall I : \TYPE{InverseJacobian}(\phi,B) \.	\NewLine \.	
		\forall g \in \Integrable(Y,T,\nu) \.
		\forall   H \in T \.
		\int_{\phi^{-1}(H)} g\big( \phi(x) \big) \; d\mu(x) = \int_H I(x)g(x) \; d \nu
	}
	\Explain{ 1 The theorem holds trivially if $g$ is simple by the definition 
		of Radon-Nikodym Jacobian}
	\Explain{ 2 Without loss of generality, consider the case $g > 0$}
