...
\subsection{Properties Preserved by Continuous Transformations}
...
\Page{
	...
	\\
	\Theorem{TotallyDisconectesByBase}
	{
		\forall X \in \TOP \.
...
\Page{   
	...
	\Theorem{CoverSeparationLemma}
	{
		...
		\forall \beth : \forall n \in \Nat \. \overline{V} \cap B = \emptyset \. \NewLine \.
		\forall \gimel : B \subset \bigcup^\infty_{n=1} W_n \.
		\forall \daleth :  \forall n \in \Nat \. \overline{W} \cap A = \emptyset \.
		\TYPE{Separated}(X,A,B)
	}
	\Explain{
		Without loss of generality assume that $V$ and $W$ are increasing}
	\Explain{
		Otherwise construct $V'_n = \bigcup^n_{k=1} V_k$ and 
		$W'_n = \bigcup^n_{k=1}  W_k$}
	\Explain{
		Clearly, these constructions inherit propertirs $\aleph',\beth',\gimel',\daleth'$}
	\Explain{
		Also without loss of generality assume that $V_n \cap \overline{W}_n = \emptyset$
		and $W_n \cap \overline{V}_n = \emptyset$}
		\Explain{
		Otherwise construct $V'_n = V_n \setminus \overline{W}_n$ and 
		$W'_n =  W_n \setminus \overline{V}_n$}
	\Explain{
		Clearly, these constructions inherit propertirs $\aleph',\beth',\gimel',\daleth'$}
	\Explain{
		Define open sets $G = \bigcap^\infty_{n=1} V_n$ and $H = \bigcap^\infty_{n=1} W_n$}
	\Explain{
		Then $A \subset G$ and $B \subset H$ by $\aleph$ and $\gimel$}
	\Explain{
		Take any $x \in G \cap H$} 
	\Explain{
		Then there exists some $n,m \in \Nat$ such that $x \in V_n \cap W_n$}
	\Explain{
		Take $k = \max(m,n)$}
	\Explain{
		Then $x \in V_k \cap W_k$ as both $V$ and $W$ are increasing}
	\Exclaim{
		But $V_k \cap \overline{W}_k = \emptyset$, so this is a contradiction}
	\Explain{
		Thus, $H \cap G = \emptyset$ and $H$ and $G$ provide the separation of
		$A$ and $B$}
	\EndProof
	\\
	\Theorem{Lindel\ddot{o}fRegularity}
	{
		\forall X : \Lindelof \And \TYPE{T3} \.
		\TYPE{T4} \And \CR(X)
	}
	\Explain{
		Assume $A$ and $B$ are both closed sets in $X$}
	\Explain{
		As $X$ is $\TYPE{T3}$ for every $a \in A$ it must be possible
		to select open $U_a$ such that $a \in U_a$ and  $\overline{U}_a \cap B = \emptyset$}
	\Explain{
		As $A$ itself is Lindel\''of it must be possible to select a countable subcover
		$\U$ from $U_A$}
	\Explain{
		By the simmilar process we may select a countable cover $\mathcal{V}$ for $B$}
	\Explain{
		But this leads us to open separation lemma, so $A$ and $B$ 
		can be separated}
	\Explain{
		Thus $X$ is $\TYPE{T4}$ and hence $\TYPE{T3.5}$ or Tychonoff}
	\EndProof
	\\
	\Theorem{Lindel\ddot{o}fFSigmaIsLinelof}
	{
		\forall X : \Compact \. 
		\forall A \in F_\sigma(X) \.
		\Lindelof(A)
	}
	\Explain{ 
		Repesent $A = \bigcup^\infty_{n=1} C_n$, where each $C_n$ is closed}
	\Explain{
			Then each $C_n$ is Lindel\"of, and if $\O$ is an open cover for $A$, then $\O$ is open cover for
			each $C_n$} 
	\Explain{ 
			Hence open countable subcovers $\O'_n$ can be selected for  each $C_n$}
	\Explain{
			Then $\O'' = \bigcup^\infty_{n=1} \O'_n$ is still countable and is a cover of $A$}
	\EndProof
}
\newpage
...
\Page{
	...
	\DefineType{\C}{FilterConvergence}
	{
		\forall x \in X \. \C\Big(x,\dot x\Big)
		\And  \NewLine \And
		\forall \mathfrak{F} \in \FILT^2(X) \.
		\exists \F \in \FILT(X) \.
		\FILT(\C)(\F,\mathfrak{F}) \And  
		\C(x,\F) 
		\Imply
		\C\Big( x,  m_X(\mathfrak{F}) \Big)
	}
	\\
	\Theorem{FilterConvergneceIsTopology}
	{
		\NewLine ::		
		\forall X \in \SET \.
		\forall  \C : \TYPE{FilterConvergence}(X) \.
		\exists! \tau : \Topology(X) \. 
		\forall (x,\F) \in X \times \FILT(X) \. \NewLine \.
		(x,\F) \in \C \iff  x \in \lim_{(X,\tau)} \F	
	}
	\Explain{ 
		Let $\C$ be a filter convergence in the sense defined above}
	\Explain{
		Define $\tau = 
			\Big\{
					U \subset X : 
					\forall (x,\F) \in \C \. 
					U(x) \Imply \langle\F\rangle(U)
			\Big\}		
		$}
	\Explain{
		Then $\emptyset$ is trivally in $\tau$}
	\Explain{
		If  $(x,\F) \in \C$, 
		then $\F \neq \emptyset$ so there are $A \subset \F$}
	\Explain{
		This means that $A \subset X \in \F$ as $\langle \F \rangle$ must be upward closed}
	\Explain{
		Assume $U:I \to \tau$ is the collection of sets,
		and $(x,\F) \in \C$ is such that $\bigcup_{i \in I} U_i(x)$}
	\Explain{
		Then there is $i \in I$ such that $U_i(x)$, so $\langle \F \rangle(U_i)$}
	\Explain{
		But as  $\langle \F \rangle$ is upwards closed 
		$\langle \F \rangle\left( \bigcup_{i \in I} U_i \right)$}
	\Explain{ 
		Thus  $\bigcup_{i \in I} U_i \in \tau$}
	\Explain{
		Now take $n \in \Nat$ and
		$U : \{1,\ldots,n\} \to \tau$ and also $(x,\F) \in C$ such that $\bigcap^n_{i=1}U_i(x)$
	}
	\Explain{
		Then $U_i(x)$ for any $i \in \{1,\ldots,n\}$, so $\langle \F \rangle(U_i)$}
	\Explain{
		But $\langle \F \rangle$	is intersection closed, so
		$\langle \F \rangle \left(\bigcap_{i=1}^n U_i\right)$}
	\Explain{
		Thus $\bigcup_{i \in I} U_i \in \tau$
		and we proved that $\tau$ is topology}
	\Explain{
		Then by construction
		$(x,\F) \in \C \Imply  x \in \lim_{(X,\tau)} \F$}
	\Explain{
		Now take $(x,\F) \in X \times \FILT(X)$, such that $x \in \lim_{(X,\tau)} \F$}
	\Explain{
		This means that $\F(U)$  for all $U \in \U(x)$}
	\Explain{
		The idea is to show that $\F = m_X(\mathfrak{F})$ for some filter of filters $\mathfrak{F}$
		and $\FILT(\C)(\dot x, \mathfrak{F})$}
	\Explain{
		So define $\mathfrak{F} = 
			\Big\{ 
			...
			\Big\}$} 
	\Explain{
		$\mathfrak{F}$ is a filterbase: it is nonempty as $\F$ is and any element of $\mathfrak{F}$
		conatains $\F$}
	\ExplainFurther{
		It is also closed by intersections as}
	\ExplainFurther{
		$
		\big\{ \langle \mathcal{G} \rangle \big|\mathcal{G} \in \FILT(X), A \in \mathcal{G} \big\} 
		\cap		
		...
		=
		\big\{ \langle \mathcal{G} \rangle \big|\mathcal{G} \in \FILT(X), A \cap B \in \mathcal{G} \big\}  		
		$}
	\Explain{
		as filters are upward and intersection closed}
	\Explain{	 
	 	Then $\F \subset m_x(\mathfrak{F})$}
}
\newpage
\Page{
	\Explain{
	...
	\Explain{
		$(\Imply)$ Assume $x,y \in X$ are two distinct limits of the ultrafilter $\F$}
	\Explain{
		Then there are disjoint $U \in \U(x)$ and $V \in \U(y)$}
	\Explain{
		But then $U,V \in \F$ but $U \cap V = \emptyset \in \F$,
		which is a contradiction}
	\Explain{
		$(\Leftarrow)$ For $x,y \in X$ there are filters
		$\N(x),\N(y) \in \FILT(X)$
		such that $x = \lim \N(x)$ and $y = \lim \N(y)$}
	\Explain{
		There are ultrafilters $\F_x,\F_y$ such that
		$\N(x) \subset \F_x$ and $\N(y) \subset \F_y$}
	\Explain{ 
		If for any pair of open neighborhoods
		$(U,V) \in \U(x) \times \U(y)$,
		then $U \in \N(y)$ and $V \in \N(x)$}
	\Explain{
		But this means that $x,y \in \lim \F_x$ 
		and $x,y \in \lim \F_y$,
...
\Page{
	...
	{
		\forall a,b : \TYPE{Net}(\U_X(x),D) \. \NewLine \.
...
One issue with compact Hausdorff spaces is that it do not have all categorical limits and colimits.
This problem may be overcomed by introducing notion of compactly generated spaces.
\Page{
