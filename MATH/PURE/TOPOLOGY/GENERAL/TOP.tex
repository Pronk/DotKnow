...
\tableofcontents
\newpage
\section{Topological Sets}
\subsection{Topology and Topological Spaces}
\Page{
...
\newpage
\subsection{Open and Closed Domains}
\Page{
...
\Page{
	...
	\NoProof
	\\
	\Theorem{DensityProductProperty}
	{
		\forall I \in \SET \.
		\forall X : I \to \TOP \.
		\forall k : \TYPE{InfiniteCardinal} \/
		\forall [0] : \forall i \in I \.  d(X_i) \le k \. \NewLine \.
		\forall U : \TYPE{Disjoint} \Act{ \T\Act{\prod_{i \in I} U_i }} \.
		|U| \le k 
	}
	\NoProof
	\\
...
\Page{
	\DeclareFunc{universal}
	{
		??\TOP \to ??\TOP
	}
	\DefineFunc{universal}{P}{\Lambda X : P \. \forall Y : P \. \exists \TYPE{HomeomorphicEmbedding}(Y,X)}
	\\
	\Theorem{TychonoffCubeIsUniversal}
	{
		\forall n : \TYPE{InfiniteCardinal} \.
		I^n : \FUNC{universal} \;  \Lambda X : \TYPE{Tychonoff}(X) \. w(X) =  n 
	}
	\NoProof
	\\
	\DeclareFunc{spaceOfAlexanroff}
	{
		\TOP
	}
	\DefineNamedFunc{spaceOfAlexadroff}{}{F}{\Big( \{0,1\},  \big\{ \emptyset, \{0\}, \{0,1\} \big\}  \Big)}
	\\
	\Theorem{AlexandroffCubeIsUniversal}
	{
		\forall n : \TYPE{InfiniteCardinal} \.
		F^n : \FUNC{universal} \;  \Lambda X : \TYPE{T1}(X) \. w(X) = n 
	}
	\NoProof
	\\
	\Theorem{ProductNetConvergence}
	{
		\forall I \in \SET \. 
		\forall X : I \to \TOP \.
		\forall D : \TYPE{DirectrdSet} \.
		\forall x : \TYPE{Net}\Act{D, \prod_{i \in I}  X_i} \.
		 \NewLine \. 
		\forall L \in \prod_{i \in I} X_i \.
		L \in \lim_{i \in D} x_i \iff \forall i \in I \. \pi_i(L) \in \lim_{i \in D} \pi_i(x_i)
	}
	\NoProof
	\\
	\Theorem{ProductNetConvergence}
	{
		\forall I \in \SET \. 
		\forall X : I \to \TOP \.
		\forall \F : \TYPE{Filter}\Act{ \prod_{i \in I}  X_i} \.
		 \NewLine \. 
		\forall L \in \prod_{i \in I} X_i \.
		L \in \lim \F \iff \forall i \in I \. \pi_i(L) \in \lim \pi_i(\F)
	}
	\NoProof
	\\
	\Theorem{TopologicalSpacesAreComplete}{\TOP : \TYPE{Complete}}
	\NoProof
}
\newpage
...
\Page{
	...
	\DefineNamedFunc{quotinentSpace}{\sim}{\frac{X}{(\sim)}}{\Act{\frac{X}{(\sim)},
		\left\{ U \subset \frac{X}{(\sim)} : \pi^{-1}_\sim (U) \in \T(X) \right\}}
	}
	\\
	\Theorem{TopologicalSpacesAreComplete}
	{
		\TOP : \TYPE{Cocomplete}
	}
	\NoProof
...
\newpage
\subsection{Perfectly Normal Spaces}
\Page{
...
\Page{
	...
	\NoProof
	\\
	\DeclareFunc{tightnessAtAPoint}
	{
		\prod X : \TOP \.
		X \to  \mathsf{CARD}
	}
	\DefineNamedFunc{tightnessAtAPoint}{x}
	{ \tau(x)  }{ \max\Big( \aleph_0, 
			\min\big\{ k \in \mathsf{CARD} : 
			\forall A \subset X : x \in A \Imply \NewLine \Imply
			\exists B \subset A : x \in \overline{B} \And |B| \le k 
			\big\} \Big)
	}
	\\
	\DeclareFunc{tightness}
	{
		\mathrm{Groupoid}(\TOP) \Arrow{\CAT} \mathsf{CARD}
	}
	\DefineNamedFunc{tightness}{X}{\tau(X)}{\sup_{x \in X} \tau(x)}
	\\
	\Theorem{TightnessDegreeInequelity}
	{
		\forall X : \TOP \.  \tau(X) \le \chi(X)
	}
	\NoProof
	\\
	\Theorem{EquivalentInequality}
	{
		\forall X : \TOP \.
		\tau(X) = \max\Big( \aleph, \NewLine ,  
		\inf \big\{ k \in \mathsf{CARD} : \forall A \IsNot \TYPE{Closed}(A), 
			\exists B \subset A : |B| \le k \overline{B} \setminus A \neq \emptyset\big\}
		\Big)
	}
	\NoProof
}
...
\newpage
\subsection{Totally Disconected Spaces}
\Page{
